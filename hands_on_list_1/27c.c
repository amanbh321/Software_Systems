...
#include <unistd.h>  // Required for the `execle` function
#include <stdio.h>   // Required for the `printf` function

int main()
{
    // Define the command path and arguments
    char *cmd_path = "/bin/ls";
    char *cmd_options = "-Rl";
    char *target_path = ".";  // Directory to list

    // Display header before executing the command
    printf("============ OUTPUT USING execle =============\n");

    // Execute the `ls` command with options and the target directory
    execle(cmd_path, cmd_path, cmd_options, target_path, NULL, NULL);

    // If execle fails, this line will execute
...
/*
============================================================================
aman@aman-HP-Laptop-15s-gr0xxx:~/Desktop/hands_on_list_1$ gcc 27c.c
...
-rw-rw-r-- 1 aman aman  3714 Aug 31 22:04 27b.c
-rw-rw-r-- 1 aman aman  1848 Aug 31 22:05 27c.c
...
-rwxrwxr-x 1 aman aman 16000 Aug 31 22:09 a.out
